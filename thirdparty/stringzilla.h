...
/**
 *  @brief  Generally `NULL` is coming from locale.h, stddef.h, stdio.h,
 * stdlib.h, string.h, time.h, and wchar.h, according to the C standard.
 */
#ifndef NULL
#define NULL ((void*)0)
#endif
...
/**
 *  @brief  Analogous to `sz_size_t` and `std::sz_size_t`, unsigned integer,
 * identical to pointer size. 64-bit on most platforms where pointers are
 * 64-bit. 32-bit on platforms where pointers are 32-bit.
 */
...
#endif
SZ_STATIC_ASSERT(
  sizeof(sz_size_t) == sizeof(void*), sz_size_t_must_be_pointer_size);
...
/**
 *  @brief  For faster bounded Levenstein (Edit) distance computation no more
 * than 255 characters are supported.
 */
...
/**
 *  @brief  Internal data-structure, used to address "anomalies" (often
 * prefixes), during substring search. Always a 32-bit unsigned integer,
 * containing 4 chars.
 */
...
/**
 *  @brief  This is a slightly faster alternative to `strncmp(a, b, length) ==
 * 0`. Doesn't provide major performance improvements, but helps avoid the LibC
 * dependency.
 *  @return 1 for `true`, and 0 for `false`.
 */
inline static sz_bool_t
sz_equal(sz_string_start_t a, sz_string_start_t b, sz_size_t length) {
...
/**
 *  @brief  Count the number of occurrences of a @b single-character needle in
 * an arbitrary length haystack. This implementation uses hardware-agnostic SWAR
 * technique, to process 8 characters at a time.
 */
inline static sz_size_t sz_count_char_swar(
  ...
  sz_string_start_t const needle) {
    ...
    // This code simulates hyper-scalar execution, comparing 8 characters at a
    // time.
...
/**
 *  @brief  Find the first occurrence of a @b single-character needle in an
 * arbitrary length haystack. This implementation uses hardware-agnostic SWAR
 * technique, to process 8 characters at a time. Identical to `memchr(haystack,
 * needle[0], haystack_length)`.
 */
inline static sz_string_start_t sz_find_1char_swar(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
    sz_string_start_t text = haystack;
    ...
    // This code simulates hyper-scalar execution, analyzing 8 offsets at a
    // time.
    sz_u64_t nnnnnnnn = *needle;
...
/**
 *  @brief  Find the last occurrence of a @b single-character needle in an
 * arbitrary length haystack. This implementation uses hardware-agnostic SWAR
 * technique, to process 8 characters at a time. Identical to `memrchr(haystack,
 * needle[0], haystack_length)`.
 */
inline static sz_string_start_t sz_rfind_1char_swar(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
    sz_string_start_t const end = haystack + haystack_length;
    ...
    for (; text - 8 >= haystack; text -= 8) {
        ...
        if (match_indicators != 0)
            return text - 8 + clz64(match_indicators) / 8;
    }
...
/**
 *  @brief  Find the first occurrence of a @b two-character needle in an
 * arbitrary length haystack. This implementation uses hardware-agnostic SWAR
 * technique, to process 8 characters at a time.
 */
inline static sz_string_start_t sz_find_2char_swar(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
    sz_string_start_t text = haystack;
    ...
    for (; ((sz_size_t)text & 7ull) && text + 2 <= end; ++text)
        if (text[0] == needle[0] && text[1] == needle[1])
            return text;

    // This code simulates hyper-scalar execution, analyzing 7 offsets at a
    // time.
    sz_u64_t nnnn
      = ((sz_u64_t)(needle[0]) << 0)
        | ((sz_u64_t)(needle[1]) << 8); // broadcast `needle` into `nnnn`
...
/**
 *  @brief  Find the first occurrence of a three-character needle in an
 * arbitrary length haystack. This implementation uses hardware-agnostic SWAR
 * technique, to process 8 characters at a time.
 */
inline static sz_string_start_t sz_find_3char_swar(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
    sz_string_start_t text = haystack;
    ...
    for (; ((sz_size_t)text & 7ull) && text + 3 <= end; ++text)
        if (
          text[0] == needle[0] && text[1] == needle[1] && text[2] == needle[2])
            return text;

    // This code simulates hyper-scalar execution, analyzing 6 offsets at a
    // time. We have two unused bytes at the end.
    ...
    for (; text + 8 <= end; text += 6) {
        ...
        first_indicators = (first_indicators >> 16) & (first_indicators >> 8)
                           & (first_indicators >> 0) & 0x0000010000010000;
        ...
        second_indicators = (second_indicators >> 16) & (second_indicators >> 8)
                            & (second_indicators >> 0) & 0x0000010000010000;
        ...
        third_indicators = (third_indicators >> 16) & (third_indicators >> 8)
                           & (third_indicators >> 0) & 0x0000010000010000;

        sz_u64_t match_indicators = first_indicators | (second_indicators >> 8)
                                    | (third_indicators >> 16);
        if (match_indicators != 0)
            return text + ctz64(match_indicators) / 8;
    }
...
/**
 *  @brief  Find the first occurrence of a @b four-character needle in an
 * arbitrary length haystack. This implementation uses hardware-agnostic SWAR
 * technique, to process 8 characters at a time.
 */
inline static sz_string_start_t sz_find_4char_swar(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
    sz_string_start_t text = haystack;
    ...
    // This code simulates hyper-scalar execution, analyzing 4 offsets at a
    // time.
    sz_u64_t nn = (sz_u64_t)(needle[0] << 0) | ((sz_u64_t)(needle[1]) << 8)
    ...
    offset_in_slice[0x2] = offset_in_slice[0x6] = offset_in_slice[0xA]
      = offset_in_slice[0xE] = 1;
    offset_in_slice[0x4] = offset_in_slice[0xC] = 2;
    offset_in_slice[0x8] = 3;

    // We can perform 5 comparisons per load, but it's easier to perform 4,
    // minimizing the size of the lookup table.
    for (; text + 8 <= end; text += 4) {
        sz_u64_t text_slice = *(sz_u64_t const*)text;
        sz_u64_t text01 = (text_slice & 0x00000000FFFFFFFF)
                          | ((text_slice & 0x000000FFFFFFFF00) << 24);
        sz_u64_t text23 = ((text_slice & 0x0000FFFFFFFF0000) >> 16)
                          | ((text_slice & 0x00FFFFFFFF000000) << 8);
        ...
        if (text01_indicators + text23_indicators) {
            // Assuming we have performed 4 comparisons, we can only have 2^4=16
            // outcomes. Which is small enough for a lookup table.
...
/**
 *  @brief  Trivial substring search with scalar SWAR code. Instead of comparing
 * characters one-by-one it compares 4-byte anomalies first, most commonly
 * prefixes. It's computationally cheaper. Matching performance fluctuates
 * between 1 GB/s and 3,5 GB/s per core.
 */
inline static sz_string_start_t sz_find_substring_swar( //
  ...
  sz_size_t const needle_length) {
    ...
    case 2:
        return sz_find_2char_swar(haystack, haystack_length, needle);
    case 3:
        return sz_find_3char_swar(haystack, haystack_length, needle);
    case 4:
        return sz_find_4char_swar(haystack, haystack_length, needle);
    default: {
        sz_string_start_t text = haystack;
        sz_string_start_t const end = haystack + haystack_length;
        ...
        while (text + needle_length <= end) {
            ...
            if (h_anomaly.u32 == n_anomaly.u32) // Match anomaly.
                if (sz_equal(
                      text + 4, n_suffix_ptr, n_suffix_len)) // Match suffix.
                    return text;
...
inline static void _sz_find_substring_populate_anomaly( //
  ...
  _sz_anomaly_t* mask_out) {
    ...
    case 1:
        ...
        anomaly.u8s[0] = needle[0],
        anomaly.u8s[1] = anomaly.u8s[2] = anomaly.u8s[3] = 0;
        break;
    case 2:
        mask.u8s[0] = mask.u8s[1] = 0xFF, mask.u8s[2] = mask.u8s[3] = 0;
        anomaly.u8s[0] = needle[0], anomaly.u8s[1] = needle[1],
        anomaly.u8s[2] = anomaly.u8s[3] = 0;
        break;
    case 3:
        mask.u8s[0] = mask.u8s[1] = mask.u8s[2] = 0xFF, mask.u8s[3] = 0;
        anomaly.u8s[0] = needle[0], anomaly.u8s[1] = needle[1],
        anomaly.u8s[2] = needle[2], anomaly.u8s[3] = 0;
        break;
    default:
        mask.u32 = 0xFFFFFFFF;
        anomaly.u8s[0] = needle[0], anomaly.u8s[1] = needle[1],
        anomaly.u8s[2] = needle[2], anomaly.u8s[3] = needle[3];
        break;
    }
    *anomaly_out = anomaly;
    *mask_out = mask;
...
/**
 *  @brief  Substring-search implementation, leveraging x86 AVX2 intrinsics and
 * speculative execution capabilities on modern CPUs. Performing 4 unaligned
 * vector loads per cycle was practically more efficient than loading once and
 * shifting around, as introduces less data dependencies.
 */
inline static sz_string_start_t sz_find_substring_avx2(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle,
  sz_size_t const needle_length) {
    ...
    while (text + needle_length + 32 <= end) {
        // Performing many unaligned loads ends up being faster than loading
        // once and shuffling around.
        __m256i texts0 = _mm256_and_si256(
          _mm256_loadu_si256((__m256i const*)(text + 0)), masks);
        int matches0 = _mm256_movemask_epi8(
          _mm256_cmpeq_epi32(texts0, anomalies));
        __m256i texts1 = _mm256_and_si256(
          _mm256_loadu_si256((__m256i const*)(text + 1)), masks);
        int matches1 = _mm256_movemask_epi8(
          _mm256_cmpeq_epi32(texts1, anomalies));
        __m256i text2 = _mm256_and_si256(
          _mm256_loadu_si256((__m256i const*)(text + 2)), masks);
        int matches2 = _mm256_movemask_epi8(
          _mm256_cmpeq_epi32(text2, anomalies));
        __m256i texts3 = _mm256_and_si256(
          _mm256_loadu_si256((__m256i const*)(text + 3)), masks);
        int matches3 = _mm256_movemask_epi8(
          _mm256_cmpeq_epi32(texts3, anomalies));
...
/**
 *  @brief  Substring-search implementation, leveraging Arm Neon intrinsics and
 * speculative execution capabilities on modern CPUs. Performing 4 unaligned
 * vector loads per cycle was practically more efficient than loading once and
 * shifting around, as introduces less data dependencies.
 */
inline static sz_string_start_t sz_find_substring_neon(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle,
  sz_size_t const needle_length) {
    ...
    while (text + needle_length + 16 <= end) {
        // Each of the following `matchesX` contains only 4 relevant bits - one
        // per word. Each signifies a match at the given offset.
        matches0 = vceqq_u32(
          vandq_u32(
            vreinterpretq_u32_u8(vld1q_u8((unsigned char*)text + 0)), masks),
          anomalies);
        matches1 = vceqq_u32(
          vandq_u32(
            vreinterpretq_u32_u8(vld1q_u8((unsigned char*)text + 1)), masks),
          anomalies);
        matches2 = vceqq_u32(
          vandq_u32(
            vreinterpretq_u32_u8(vld1q_u8((unsigned char*)text + 2)), masks),
          anomalies);
        matches3 = vceqq_u32(
          vandq_u32(
            vreinterpretq_u32_u8(vld1q_u8((unsigned char*)text + 3)), masks),
          anomalies);
        matches = vorrq_u32(
          vorrq_u32(matches0, matches1), vorrq_u32(matches2, matches3));
...
inline static sz_size_t sz_count_char(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
...
inline static sz_string_start_t sz_find_1char(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
    return sz_find_1char_swar(haystack, haystack_length, needle);
...
inline static sz_string_start_t sz_rfind_1char(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle) {
...
inline static sz_string_start_t sz_find_substring(
  sz_string_start_t const haystack,
  sz_size_t const haystack_length,
  sz_string_start_t const needle,
  sz_size_t const needle_length) {
    if (haystack_length < needle_length || needle_length == 0)
        return NULL;
#if defined(__ARM_NEON)
    return sz_find_substring_neon(
      haystack, haystack_length, needle, needle_length);
#elif defined(__AVX2__)
    return sz_find_substring_avx2(
      haystack, haystack_length, needle, needle_length);
#else
    return sz_find_substring_swar(
      haystack, haystack_length, needle, needle_length);
#endif
}

/**
 *  @brief  Maps any ASCII character to itself, or the lowercase variant, if
 * available.
 */
...
/**
 *  @brief  Maps any ASCII character to itself, or the uppercase variant, if
 * available.
 */
 ...
 *        to achieve the computation. It's designed to work with GCC/Clang and
 * MSVC.
 */
inline static sz_size_t sz_log2i(sz_size_t n) {
    ...
    if (_BitScanReverse64(&index, n))
        return index;
    return 0; // This line might be redundant due to the initial check, but it's
              // safer to include it.
#else
...
#endif
#else
// Handle non-Windows platforms. You can further differentiate between 32-bit
// and 64-bit if needed.
#if defined(__LP64__)
    return 63 - __builtin_clzll(n);
#else
    return 31 - __builtin_clz(n);
 ...
 *          Doesn't provide major performance improvements, but helps avoid the
 * LibC dependency.
 */
inline static sz_bool_t sz_is_less_ascii(
  sz_string_start_t a,
  ...
  sz_size_t const b_length) {
    ...
    while (a + 8 <= min_end
           && sz_u64_unaligned_load(a) == sz_u64_unaligned_load(b))
        a += 8, b += 8;
    while (a != min_end && *a == *b)
        a++, b++;
...
/**
 *  @brief  Char-level lexicographic comparison of two strings, insensitive to
 * the case of ASCII symbols. Doesn't provide major performance improvements,
 * but helps avoid the LibC dependency.
 */
inline static sz_bool_t sz_is_less_uncased_ascii(
...
/**
 *  @brief  Helper, that swaps two 64-bit integers representing the order of
...
typedef sz_string_start_t (*sz_sequence_member_start_t)(
  struct sz_sequence_t const*, sz_size_t);
typedef sz_size_t (*sz_sequence_member_length_t)(
  struct sz_sequence_t const*, sz_size_t);
typedef sz_bool_t (*sz_sequence_predicate_t)(
  struct sz_sequence_t const*, sz_size_t);
typedef sz_bool_t (*sz_sequence_comparator_t)(
  struct sz_sequence_t const*, sz_size_t, sz_size_t);
typedef sz_bool_t (*sz_string_is_less_t)(
  sz_string_start_t, sz_size_t, sz_string_start_t, sz_size_t);
...
/**
 *  @brief  Similar to `std::partition`, given a predicate splits the sequence
 * into two parts. The algorithm is unstable, meaning that elements may change
 * relative order, as long as they are in the right partition. This is the
 * simpler algorithm for partitioning.
 */
inline static sz_size_t
sz_partition(sz_sequence_t* sequence, sz_sequence_predicate_t predicate) {
    sz_size_t matches = 0;
    while (matches != sequence->count
           && predicate(sequence, sequence->order[matches]))
        ++matches;

    for (sz_size_t i = matches + 1; i < sequence->count; ++i)
        if (predicate(sequence, sequence->order[i]))
            _sz_swap_order(sequence->order + i, sequence->order + matches),
              ++matches;
...
/**
 *  @brief  Inplace `std::set_union` for two consecutive chunks forming the same
 * continuous `sequence`.
 *
 *  @param partition The number of elements in the first sub-sequence in
 * `sequence`.
 *  @param less Comparison function, to determine the lexicographic ordering.
 */
inline static void sz_merge(
  sz_sequence_t* sequence, sz_size_t partition, sz_sequence_comparator_t less) {
    ...
    while (start_a <= partition && start_b <= sequence->count) {
        ...
        if (!less(
              sequence, sequence->order[start_b], sequence->order[start_a])) {
            start_a++;
        } else {
...
inline static void
sz_sort_insertion(sz_sequence_t* sequence, sz_sequence_comparator_t less) {
...
inline static void _sz_sift_down(
  ...
  sz_size_t end) {
    ...
    while (2 * root + 1 <= end) {
        ...
        if (
          child + 1 <= end && less(sequence, order[child], order[child + 1])) {
            child++;
        }
...
inline static void _sz_heapify(
  sz_sequence_t* sequence,
  sz_sequence_comparator_t less,
  sz_u64_t* order,
  sz_size_t count) {
...
inline static void _sz_heapsort(
  sz_sequence_t* sequence,
  sz_sequence_comparator_t less,
  sz_size_t first,
  sz_size_t last) {
  ...
  sz_size_t depth) {
    ...
    case 2:
        if (less(sequence, sequence->order[first + 1], sequence->order[first]))
            _sz_swap_order(
              &sequence->order[first], &sequence->order[first + 1]);
        return;
    case 3: {
...
inline static void
sz_sort_introsort(sz_sequence_t* sequence, sz_sequence_comparator_t less) {
...
inline static void _sz_sort_recursion( //
  ...
  sz_size_t partial_order_length) {
    ...
    sz_size_t split = 0;
    {
        ...
        for (sz_size_t i = split + 1; i < sequence->count; ++i)
            if (!(sequence->order[i] & mask))
                _sz_swap_order(sequence->order + i, sequence->order + split),
                  ++split;
    }
    ...
    if (bit_idx < bit_max) {
        ...
        a.count = split;
        _sz_sort_recursion(
          &a, bit_idx + 1, bit_max, comparator, partial_order_length);

        sz_sequence_t b = *sequence;
        b.order += split;
        b.count -= split;
        _sz_sort_recursion(
          &b, bit_idx + 1, bit_max, comparator, partial_order_length);
    }
...
inline static sz_bool_t _sz_sort_compare_less_ascii(
  sz_sequence_t* sequence, sz_size_t i_key, sz_size_t j_key) {
...
inline static sz_bool_t _sz_sort_compare_less_uncased_ascii(
  sz_sequence_t* sequence, sz_size_t i_key, sz_size_t j_key) {
    sz_string_start_t i_str = sequence->get_start(sequence, i_key);
    sz_size_t i_len = sequence->get_length(sequence, i_key);
    sz_string_start_t j_str = sequence->get_start(sequence, j_key);
...
/**
 *  @brief  Sorting algorithm, combining Radix Sort for the first 32 bits of
 * every word and a follow-up by a more conventional sorting procedure on
 * equally prefixed parts.
 */
inline static void
sz_sort(sz_sequence_t* sequence, sz_sort_config_t const* config) {
    sz_bool_t case_insensitive = config && config->case_insensitive;
    sz_size_t partial_order_length = config && config->partial_order_length
                                       ? config->partial_order_length
                                       : sequence->count;

    // Export up to 4 bytes into the `sequence` bits themselves
    for (sz_size_t i = 0; i != sequence->count; ++i) {
        sz_string_start_t begin = sequence->get_start(
    ...
    sz_sequence_comparator_t comparator = (sz_sequence_comparator_t)
      _sz_sort_compare_less_ascii;
    if (case_insensitive)
        comparator = (sz_sequence_comparator_t)
          _sz_sort_compare_less_uncased_ascii;
 ...
 */
inline static sz_size_t
sz_levenstein_memory_needed(sz_size_t _, sz_size_t b_length) {
...
/**
 *  @brief  Levenshtein String Similarity function, implemented with linear
 * memory consumption. It accepts an upper bound on the possible error.
 * Quadratic complexity in time, linear in space.
 */
inline static levenstein_distance_t sz_levenstein( //
  ...
  void* buffer) {
    ...
    // If the difference in length is beyond the `bound`, there is no need to
    // check at all
    ...
    levenstein_distance_t* current_distances = previous_distances + b_length
                                               + 1;
    ...
    for (sz_size_t idx_a = 0; idx_a != a_length; ++idx_a) {
        ...
        for (sz_size_t idx_b = 0; idx_b != b_length; ++idx_b) {
            levenstein_distance_t cost_deletion = previous_distances[idx_b + 1]
                                                  + 1;
            levenstein_distance_t cost_insertion = current_distances[idx_b] + 1;
            levenstein_distance_t cost_substitution = previous_distances[idx_b]
                                                      + (a[idx_a] != b[idx_b]);
            current_distances[idx_b + 1] = _sz_levenstein_minimum(
              cost_deletion, cost_insertion, cost_substitution);
    ...
    return previous_distances[b_length] <= bound ? previous_distances[b_length]
                                                 : bound;
}

inline static sz_u32_t
sz_hash_crc32_swar(sz_string_start_t start, sz_size_t length) {
    /*
...
#if defined(__ARM_FEATURE_CRC32)
inline static sz_u32_t
sz_hash_crc32_arm(sz_string_start_t start, sz_size_t length) {
    sz_u32_t crc = 0xFFFFFFFF;
    sz_string_start_t const end = start + length;

    // Align the input to the word boundary
    while (((unsigned long)start & 7ull) && start != end) {
        crc = __crc32cb(crc, *start), start++;
    }

    // Process the body 8 bytes at a time
    while (start + 8 <= end) {
        crc = __crc32cd(crc, *(unsigned long long*)start), start += 8;
    }

    // Process the tail bytes
    if (start + 4 <= end) {
        crc = __crc32cw(crc, *(unsigned int*)start), start += 4;
    }
    if (start + 2 <= end) {
        crc = __crc32ch(crc, *(unsigned short*)start), start += 2;
    }
    if (start < end) {
        crc = __crc32cb(crc, *start);
    }
    return crc ^ 0xFFFFFFFF;
}
#endif
...
inline static sz_u32_t
sz_hash_crc32_sse(sz_string_start_t start, sz_size_t length) {
    sz_u32_t crc = 0xFFFFFFFF;
    sz_string_start_t const end = start + length;

    // Align the input to the word boundary
    while (((unsigned long)start & 7ull) && start != end) {
        crc = _mm_crc32_u8(crc, *start), start++;
    }

    // Process the body 8 bytes at a time
    while (start + 8 <= end) {
        crc = (sz_u32_t)_mm_crc32_u64(crc, *(unsigned long long*)start),
        start += 8;
    }

    // Process the tail bytes
    if (start + 4 <= end) {
        crc = _mm_crc32_u32(crc, *(unsigned int*)start), start += 4;
    }
    if (start + 2 <= end) {
        crc = _mm_crc32_u16(crc, *(unsigned short*)start), start += 2;
    }
    if (start < end) {
...
/**
 *  @brief  Hashes provided string using hardware-accelerated CRC32
 * instructions.
 */
inline static sz_u32_t
sz_hash_crc32(sz_string_start_t start, sz_size_t length) {
#if defined(__ARM_FEATURE_CRC32)
